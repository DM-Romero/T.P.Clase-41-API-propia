...
const moviesController = {
    ...
    try {
      ...
      */
      ...
      return res.status(error.status || 500).json({
        ok: false,
        msg: error.message || "Upss, hubo un ERROR. Llamá a ElMarti",
      });
    }
  },
  ...
  detail: async (req, res) => {
    try {
      if (isNaN(req.params.id)) throw createError("No es un ID valido")
  ...
  create: async function (req, res) {
    try {
      ...
      //if(!title || title === "") throw new Error("El title es obligatorio")
      //if(!rating || rating === "") throw new Error("El rating es obligatorio")


      if ([title, rating, awards, release_date, genre_id, length].includes("" || undefined))
        throw createError
      (400,"HAY UN ERROR Todos los campos son obligatorios");
  ...
  update: async function (req, res) {
    ...
    try {
      ...
      if ([title, rating, awards, release_date, genre_id, length].includes("" || undefined)) throw createError
      if (isNaN(req.params.id)) throw createError("No es un ID valido")
